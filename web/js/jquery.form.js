...
$.fn.attr2 = function() {
    ...
    if (val?.jquery || typeof val === 'string') 
        return val;
...
$.fn.ajaxSubmit = function(options) {
    ...
    var $form = this;
    options = normalizeOptions(options, $form);

    if (shouldAbortSubmission($form, options)) {
        return this;
    }

    var formData = prepareFormData($form, options);
    options = prepareAjaxOptions($form, options, formData);

    var jqxhr = handleFileUploadOrAjax($form, options, formData);
    $form.removeData('jqxhr').data('jqxhr', jqxhr);

    // Fire form-submit-notify event
...
function normalizeOptions(options, $form) {
    ...
    var url = action || window.location.href.split('#')[0];
...
function prepareFormData($form, options) {
    ...
    var elements = [];
    var formData = $form.formToArray(options.semantic, elements);

    if (options.data) {
        options.extraData = options.data;
        options.serializedData = $.param(options.data, traditional);
    }
...
function prepareAjaxOptions($form, options, formData) {
    ...
    if (options.serializedData) {
        q = q ? (q + '&' + options.serializedData) : options.serializedData;
    }
    ...
    options.success = function(data, status, xhr) {
        ...
        executeCallbacks(context, options.successCallbacks, data, status, xhr || $form, $form);
    };
...
function handleFileUploadOrAjax($form, options, formData) {
    ...
    var isMultipart = ($form.attr('enctype') === 'multipart/form-data' || $form.attr('encoding') === 'multipart/form-data');
    var fileAPI = feature.fileapi && feature.formdata;

    if (options.iframe !== false && (options.iframe || (hasFileInputs && !fileAPI))) {
        return handleFileUploadIframe($form, options, formData);
    } else if ((hasFileInputs || isMultipart) && fileAPI) {
        return handleFileUploadXhr($form, options, formData);
    } else {
        return $.ajax(options);
    }
}
...
function handleFileUploadXhr($form, options, formData) {
    ...
    if (options.extraData) {
        ...
        serializedData.forEach(item => formdata.append(item[0], item[1]));
    }
        ...
        settings.xhr = function() {
            ...
            if (xhr.upload) {
                xhr.upload.addEventListener('progress', function(event) {
                    ...
                    options.uploadProgress(event, event.loaded, event.total, percent);
                }, false);
            }
            return xhr;
        };
